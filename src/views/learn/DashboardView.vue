...
<template>
  <PageWrapper>
    <div class="mb-8">
      ...
      <p class="mt-1 text-sm text-gray-500">Continue de onde parou</p>
    </div>

    <!-- Skeletons -->
    <div v-if="loading" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <div v-for="n in 3" :key="n" class="animate-pulse rounded-xl border border-gray-200 bg-white p-4">
        <div class="mb-3 h-36 rounded-lg bg-gray-200" />
    ...
    <div v-else-if="enrollments.length === 0" class="flex flex-col items-center gap-4 py-20 text-center">
      ...
      <div>
        <p class="text-lg font-medium text-gray-800">Você ainda não tem cursos</p>
        <p class="mt-1 text-sm text-gray-500">Explore o catálogo e comece a aprender</p>
      </div>
          ...
          <div class="flex-1">
            <div class="flex items-start justify-between gap-2">
              ...
              <span
                ...
                class="flex-shrink-0 rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700"
              >
                Concluído
              </span>
            </div>
            <p v-if="enrollment.course.instructor_name" class="mt-1 text-sm text-gray-500">
              {{ enrollment.course.instructor_name }}
            </p>
            <p v-if="enrollment.course.difficulty" class="mt-0.5 text-xs text-gray-400">
